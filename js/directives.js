		    ...
		    function onRemove(e,ui) {
				...
				function message(){
					...
					scope.messageText = hasInvalidType
						? "Vous devriez seulement utiliser " + scope.types.map(function (d){ return d.name.toLowerCase() + "s"; }).join(" or ") + " here"
						: "glisser " + scope.types.map(function (d){ return d.name.toLowerCase() + "s"; }).join(", ") + " ici";
    ...
    link: function postLink(scope, element, attrs) {
    	...
    	function update(){
    		...
    		if(!scope.data|| !scope.data.length) {
    			d3.select(element[0]).append("span").text("S'il vous plaît, examinez vos données")
    			return;
    		}
    ...
    return {
      ...
      template :  '<div class="row">' +
                    '<form class="form-search col-lg-12">' +
                      '<button bs-select class="btn btn-default" placeholder="Choisir type" ng-model="mode" bs-options="m.label for m in modes">' +
