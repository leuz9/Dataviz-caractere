...
angular.module('raw.controllers', [])
  ...
  .controller('RawCtrl', function ($scope, dataService, $http, $timeout, $sce) {
    ...
    $scope.$watch('url', function (url) {
      ...
      if (is.not.url(url)) {
        $scope.error = "Veuillez insérer une URL valide";
        return;
      }
      ...
      $http.jsonp($sce.trustAsResourceUrl(url), {jsonpCallbackParam: 'callback'})
          ...
          function(response){
            $scope.loading = false;
            $scope.error = "Quelque chose ne va pas avec l'URL que vous avez fournie. S'il vous plaît assurez-vous que c'est la bonne adresse.";
          }
        )
